...
{
	float r = rgb.x;
	float MeshObject = rgb.y;
	float b = rgb.z;
	float y = 0.2126 * r + 0.7152 * MeshObject + 0.0722 * b;
	float u = -0.09991 * r - 0.33609 * MeshObject + 0.436 * b;
	float v = 0.615 * r - 0.55861 * MeshObject - 0.05639 * b;
	return float3(y, u, v);
}
...
float3 YUV2RGB(float3 yuv)
{
	...
	float v = yuv.z;
	float r = y + 1.28033 * v;
	float MeshObject = y - 0.21482 * u - 0.38059 * v;
	float b = y + 2.12798 * u;
	return float3(r, MeshObject, b);
}
